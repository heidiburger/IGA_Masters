...
%     B(2:end-1)=linspace(x_start+1/(p*el_num),R-(p*el_num-1)/(p*el_num),n-2);      %Control polygon
    const_w=2*ones(size(B));            %Weights associated with control points. Weights constant to convert to B-splines
...
%Initialise Matrices
    ...
    xi_check=zeros(1,el_num*length(Xi_bar));
    xi_count=0;
%Main Loops
    for el=1:el_num
...
%         % NURBS code to determine value of Gauss point in physical domain  
            r=basis_functR1(p,Xi,n,xi,const_w,B);

            K_e = K_e + JXi*JXi_bar*w(gp)*k*r*(N_dx*N_dx');
            F_e = F_e + JXi*JXi_bar*w(gp)*s*r*N';
...
%Draw field variable polygon
    c_vec=basis_functR1(p,Xi,n,xi_vec,const_w,d);    
%Draw x-vec against which to plot field variable
    x_vec=basis_functR1(p,Xi,n,xi_vec,const_w,B);
