...
for e = 1:n_elem
    ...
    for I = e_support(e,1):e_support(e,2)
        for J = e_support(e,1):e_support(e,2)
            ...
            sum = 0;
            for j = 1:no_GP
                %note J2 just set as 1/4. Doesn't matter because also in
                %F
                sum = sum + 1/4*basis_funct_deriv(p,knot_vec,I,xi_GPs(j))*basis_funct_deriv(p,knot_vec,J,xi_GPs(j))*weights(j);
            end
        ...
        sum1 = 0;
        for jj = 1:no_GP
            sum1 = sum1 + 1/4*xi_GPs(jj)*basis_funct(p,knot_vec,I,xi_GPs(jj))*weights(jj);
        end
